...
async function submitForm() {
  ...
  try {
    ...
    submitError.value = error?.data?.message || 'Unable to submit right now. Please try email or WhatsApp.'
  }
  finally {
...
<template>
  <section id="consult" class="bg-[#17342d] py-14 text-white md:py-18">
    <div class="container mx-auto grid gap-8 px-4 lg:grid-cols-[0.9fr_1.1fr]">
      ...
      <form class="rounded-lg bg-white p-5 text-[#17342d] shadow-xl md:p-6" @submit.prevent="submitForm">
        <div class="grid gap-4 md:grid-cols-2">
          <label class="block">
            <span class="mb-2 block text-sm font-semibold text-[#4f5f59]">Name</span>
            <input v-model="form.name" required class="w-full rounded-md border border-[#c9d8d1] px-3 py-3" placeholder="Your name" />
          </label>
          ...
          <label class="block">
            ...
            <input v-model="form.contact" required class="w-full rounded-md border border-[#c9d8d1] px-3 py-3" placeholder="How should we reply?" />
          </label>
        ...
        <p v-if="submitSuccess" class="mt-4 rounded-md bg-[#e7f2ed] px-4 py-3 text-sm font-semibold text-[#245c4c]">
          Thanks, your request has been saved. We will contact you soon.
        </p>
        <p v-if="submitError" class="mt-4 rounded-md bg-[#fff2f0] px-4 py-3 text-sm font-semibold text-[#9b2c1f]">
          {{ submitError }}
        </p>

        <div class="mt-5 flex flex-col gap-3 sm:flex-row">
          <button type="submit" :disabled="isSubmitting" class="rounded-md bg-[#0f5f4c] px-5 py-3 text-center font-semibold text-white hover:bg-[#0b4336] disabled:cursor-not-allowed disabled:bg-[#7aa699]">
            {{ isSubmitting ? 'Sending...' : 'Submit request' }}
          </button>
          <a :href="mailtoLink" class="rounded-md border border-[#b6cdc3] px-5 py-3 text-center font-semibold text-[#17342d] hover:border-[#0f5f4c]">
            Send by email
          </a>
